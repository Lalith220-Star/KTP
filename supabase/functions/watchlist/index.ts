...
export default async function (req: Request) {
  try {
    ...
    if (method === 'GET') {
      ...
      return new Response(JSON.stringify({ watchlist: data }), { status: 200 });
    }

    const body = await req.json();
    if (method === 'POST') {
      const { restaurant_id } = body;
      if (!restaurant_id) return new Response(JSON.stringify({ error: 'restaurant_id required' }), { status: 400 });
      const { data, error } = await supabase.from('watchlists').insert({ user_id: userId, restaurant_id }).select().single();
      if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500 });
      return new Response(JSON.stringify({ added: data }), { status: 201 });
    }
    ...
    if (method === 'DELETE') {
      ...
      if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500 });
      return new Response(JSON.stringify({ removed: restaurant_id }), { status: 200 });
    }
